...
<script>
    ...
    function loadGridData() {
        fetch("{{ route('gestione_turni_presse.json') }}") // <-- GET
            .then(r => {
    ...
    function createItem() {
        ...
        $('#ModalManage .modal-body').load("{{ route('gestione_turni_presse.create') }}/", function(){
    ...
    function updateItem(id) {
        $('#ModalManage .modal-body').html("");
        $('#ModalManage .modal-body').load("{{ route('gestione_turni_presse.create') }}/" + id, function(){
            afterFormLoaded();                  // <--- e qui!
        });
        $('#ModalManage').modal('show');
    }
    ...
    function deleteItem(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post("{{ route('gestione_turni_presse.destroy') }}", {
                    id_to_del: id,
                    _token: "{{ csrf_token() }}"
                }).done(function () {
                    loadGridData();
